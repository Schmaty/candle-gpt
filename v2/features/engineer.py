...
def compute_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Keep the v2.0 schema positions for checkpoint compatibility, but do not
    # feed absolute price level or monotonically increasing index into new
    # training/inference. Those columns made it too easy for a high-capacity
    # model to key/memorize the historical timeline when windows overlap.
    log_close = pd.Series(0.0, index=df.index)
    time_index_norm = pd.Series(0.0, index=df.index)

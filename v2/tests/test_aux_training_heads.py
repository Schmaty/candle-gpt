import numpy as np
import pandas as pd
from pathlib import Path

import torch

from v2.data.store import write_klines, write_funding, write_liq_bucketed
from v2.features.constants import N_FEATURES
from v2.model.config import ModelConfig
from v2.model.model import CandleGPTv2
from v2.train.config import TrainConfig
from v2.train.loop import train


def test_forward_can_return_hidden_without_changing_default_output():
    cfg = ModelConfig(n_features=N_FEATURES, d_model=32, n_heads=4, n_layers=1, block_size=16, n_bins=16)
    model = CandleGPTv2(cfg)
    x = torch.randn(2, 16, N_FEATURES)
    logits = model(x)
    logits2, hidden = model(x, return_hidden=True)
    assert logits.shape == (2, 16, 16)
    assert logits2.shape == logits.shape
    assert hidden.shape == (2, 16, 32)


def _write_synthetic_klines(path: Path, n: int = 20000) -> None:
    rng = np.random.default_rng(2)
    close = 100.0 * np.cumprod(1 + rng.normal(0, 0.002, n))
    regime_arr = np.array([0] * (n // 3) + [1] * (n // 3) + [2] * (n - 2 * (n // 3)), dtype=np.int8)
    df = pd.DataFrame({
        "open_time":  np.arange(n, dtype=np.int64) * 60_000,
        "open":       close * (1 + rng.normal(0, 0.001, n)),
        "high":       close * (1 + np.abs(rng.normal(0, 0.002, n))),
        "low":        close * (1 - np.abs(rng.normal(0, 0.002, n))),
        "close":      close,
        "volume":     np.abs(rng.normal(1000, 200, n)),
        "close_time": np.arange(n, dtype=np.int64) * 60_000 + 59_999,
        "regime":     regime_arr,
    })
    write_klines(df, path)


def _write_synthetic_funding(path: Path) -> None:
    df = pd.DataFrame({
        "funding_time": np.array([i * 8 * 60 * 60 * 1000 for i in range(10)], dtype=np.int64),
        "funding_rate": np.full(10, 0.0001, dtype=np.float64),
        "mark_price":   np.full(10, 100.0, dtype=np.float64),
    })
    write_funding(df, path)


def _write_synthetic_liq(path: Path, n: int = 20000) -> None:
    df = pd.DataFrame({
        "bucket_time":       np.arange(n, dtype=np.int64) * 60_000,
        "count":             np.zeros(n, dtype=np.int64),
        "sum_notional":      np.zeros(n, dtype=np.float64),
        "max_single":        np.zeros(n, dtype=np.float64),
        "long_liq_count":    np.zeros(n, dtype=np.int64),
        "long_liq_notional": np.zeros(n, dtype=np.float64),
        "short_liq_count":   np.zeros(n, dtype=np.int64),
        "short_liq_notional":np.zeros(n, dtype=np.float64),
    })
    write_liq_bucketed(df, path)


def test_smoke_training_with_aux_heads(tmp_path: Path):
    raw_dir = tmp_path / "raw"
    raw_dir.mkdir()
    _write_synthetic_klines(raw_dir / "btcusdt_1m.parquet")
    _write_synthetic_funding(raw_dir / "funding_btcusdt.parquet")
    _write_synthetic_liq(raw_dir / "liq_btcusdt_per_minute.parquet")

    cfg = TrainConfig(
        raw_dir=raw_dir,
        runs_dir=tmp_path / "runs",
        model=ModelConfig(
            n_features=N_FEATURES, d_model=32, n_heads=4, n_layers=1,
            block_size=32, n_bins=16, dropout=0.0,
        ),
        window=32,
        stride_train=16,
        stride_val=32,
        batch_size=4,
        max_steps=6,
        warmup_steps=2,
        val_interval_steps=3,
        val_batches=1,
        log_interval_steps=3,
        n_bins=16,
        aux_return_loss_weight=0.05,
        aux_direction_loss_weight=0.05,
        max_wall_clock_s=3600.0,
        checkpoint_interval_s=3600.0,
        progress_interval_s=0.0,
    )

    train(cfg)
    ckpt = torch.load(cfg.best_ckpt_path, map_location="cpu", weights_only=False)
    assert "aux_heads_state" in ckpt
    assert cfg.best_ckpt_path.exists()
